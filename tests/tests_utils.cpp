...
TEST(Utils, ParseSizeArgNormal)
{
    t_ping_options ping_options;

    const char *values[] = {"56", NULL};
    t_argr argr = {
        .values = (char **)values,
        .option = NULL};
    ASSERT_EQ(parse_size_arg(&ping_options, &argr, "ft_ping"), 0);
    ASSERT_EQ(ping_options.size, 56);
}

TEST(Utils, ParseSizeArgInvalid)
{
    t_ping_options ping_options;
    ...
    t_argr argr = {
        ...
        .option = NULL};
    ::testing::internal::CaptureStdout();
    ASSERT_NE(parse_size_arg(&ping_options, &argr, "ft_ping"), 0);
    std::string output = ::testing::internal::GetCapturedStdout();
    ASSERT_STREQ(output.c_str(), "ft_ping: invalid size: 'invalid'\n");
}